...
public class UpdateEventCommand extends Command {
     ...
     */
    public UpdateEventCommand(String arguments) throws DukeException {
        super();
        try {
        ...
        if (arguments.contains("/date") && arguments.split("/date ").length > 1) {
            ...
            this.at = dateOnly.trim().length() == 0 ? null : dateOnly.trim(); // Don't assign if it is an empty string.
        }
        if (this.taskName == null && this.at == null) {
            throw new DukeException("Please enter details, date or both for me to update!");
        }
    }

    /**
     * Executes the command by updating the specified Event appropriately.
     *
    ...
    public String execute(TaskList taskList, Storage storage) throws DukeException {
        ...
        if (taskName == null) {
            taskList.update(taskNo, new Event(task.getTaskName(), at));
        } else if (at == null) {
            taskList.update(taskNo, new Event(taskName, ((Event) task).getDate()));
        } else {
            taskList.update(taskNo, new Event(taskName, at));
        }
