...
public class TaskList {
     ...
     * @throws IncorrectUserFormatException If the user did not provide a name for the task.
     */
    public Todo addTodo(String name) throws IncorrectUserFormatException {
        if (name.length() == 0 || name.isBlank()) {
            throw new IncorrectUserFormatException("No name was provided for the todo item.");
        } else {
     ...
     * @return The task added to taskList.
     * @throws IncorrectUserFormatException If the user did not provide a name or time for the task.
     */
    public Event addEvent(String name, String time) throws IncorrectUserFormatException {
        if (name.length() == 0 || name.isBlank()) {
            throw new IncorrectUserFormatException("No name was provided for the event.");
        } else if (time.length() == 0 || time.isBlank()) {
            throw new IncorrectUserFormatException("No time was provided for the event.");
        } else {
     ...
     * @throws IncorrectUserFormatException If the user did not provide a name or time for the task.
     */
    public Deadline addDeadline(String name, String time) throws IncorrectUserFormatException {
        if (name.length() == 0 || name.isBlank()) {
            throw new IncorrectUserFormatException("No name was provided for the deadline item.");
        } else if ( time.length() == 0 || time.isBlank()) {
            throw new IncorrectUserFormatException("No time was provided for the deadline item.");
        } else {
     ...
     * @throws TaskOutOfBoundsException If there is no task in taskList with the given number.
     */
    public Task getTaskByNum(int i) throws TaskOutOfBoundsException {
