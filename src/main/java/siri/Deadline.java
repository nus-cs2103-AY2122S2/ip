...
/**
 * Class for the deadline items of the tasklist, with int done to be indicative of whether the task had been
 * completed, String item to be the name of the task, LocalDate eventDate as the due date for the deadline
 * task (and LocalTime eventTime as due time for the deadline task).
 */
class Deadline extends Task {
     ...
     * @return true if date is the same as the Deadline task date and false if the date is not the same as the
     * Deadline task date (ignores format).
     */
     ...
     *
     * @return String representation of the event task details, including E to represent event task,
     * whether it is done and task name, and date and time of event occurance.
     */
    @Override
    ...
    public String saveData() {
        ...
        if (this.dueTime == null) {
            dataString = "D " + String.valueOf(this.isDone) + " " + this.item
                    + " /by " + this.dueDate.format(dateFormat);
        } else {
            dataString = "D " + String.valueOf(this.isDone) + " " + this.item
                    + " /by " + this.dueDate.format(dateFormat) + " " + this.dueTime.format(timeFormat);
        }
        return dataString;
    }
}
